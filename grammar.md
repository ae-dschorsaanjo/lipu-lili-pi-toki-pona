...
The grammar and the dictionary is mostly based on (and 100% compatible with) _pu_, but also extended and clarified based on other sources (_jan Pije's_ lessons, _jan Kipo's_ writings, _common practices_, etc) and my own interpretations as well. 
...
The word order is SVO (subject-verb-object) or SliVeO (subject-_li_-verb-_e_-object), and prepositions are after the object. The _usual prepositions_ (kepeken, lon, tawa) can be placed right after the subject (but _before li_) as well, with an additional comma before them if necessary. Adjectives, adverbs (commonly _modifiers_) and possessive is _after_ the noun or verb!
...
Although normally shortenings would not occur, I recomment to use _double quotes_ `"..."` always for quotes to avoid ambiguity with them (where _apostrophes_ `'` might be used).  
...
The usual meaning is _located in/on/at_, and _existing_ as an adjective. It usually does not used as a verb, but it could be with the meaning _place in/on_ or _put into/onto/_. lon can also mark _time_ ("lon tenpo ni" -- now), although using a _la structure_ is recommended.
...
pi is also used for _relative clauses_ sometimes (mi lukin e jan pi moku e kala -- I see a man who eats a fish): this practice is not recommended and technically incorrect.  
...
In special cases, where one would use an "and", but toki pona usually does not give an easy solution for that, I recommend using "en". This use is **very debatable** and **falls in the gray area**.
...
Unofficial words (a.k.a. _proper names_) are usually used as only adjectives, only after a noun. I believe that in many cases the noun is ommitable, especially if it would be repeated or when the noun is in the subject while the proper name is in the object, for example:
...
- (when talking about someone) _ona li Sopija_ (she's Sophie) instead of _ona li jan Sopija_ -- by context it is obvious that the noun is _jan_.

For obvious reasons, _do not_ use official words are personal names! (e.g. jan Kala, jan Waso...)
